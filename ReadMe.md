...
**`./floability-env-cli.sh  --notebook {notebook_path} --kernel {kernel_name} --password {sudo_password}`**  


`floability-env.cli` currently works in the following steps: 

**1.**	Adds code to strace the kernel at the top of the notebook and adds code at the bottom to stop the stracing  
**2.**	Launches jupyter kernel with the given notebook
**3.**	Run the given notebook with the provided kernel.  
**4.**	Close the jupyter notebook command with Ctrl+C from the terminal which will be trapped by our script
**5.**	Parse the audit logs generated by strace and generates requirements.txt file 
